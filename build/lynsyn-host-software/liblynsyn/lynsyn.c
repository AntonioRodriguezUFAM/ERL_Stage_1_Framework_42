...
#define USB_IO_TIMEOUT_MS 1000  // NEW: sane default, tweak if needed
...
//** int lynsyn_reset_device(); */
// === lynsyn_reset_device (ROBUST RE-OPEN) ===
// // OLD:
// int lynsyn_reset_device() {
//     if (!lynsynHandle) {
//         return -1;
//     }
//     return libusb_reset_device(lynsynHandle);
// }

// NEW:
int lynsyn_reset_device() {
  if (!lynsynHandle) return -1;

  // Drop interface before reset (best practice)         // NEW
  libusb_release_interface(lynsynHandle, 0x1);           // NEW

  int ret = libusb_reset_device(lynsynHandle);           // NEW
  if (ret != LIBUSB_SUCCESS) {
    printf("libusb_reset_device failed: %s\n", libusb_error_name(ret)); // NEW
    return ret;
  }

  // Close and reopen to refresh handle state            // NEW
  libusb_close(lynsynHandle);                            // NEW
  lynsynHandle = libusb_open_device_with_vid_pid(usbContext, 0x10c4, 0x8c1e); // NEW
  if (!lynsynHandle) {
    printf("Re-open after reset failed\n");              // NEW
    return LIBUSB_ERROR_NO_DEVICE;                       // NEW
  }

  libusb_set_auto_detach_kernel_driver(lynsynHandle, 1); // NEW
  ret = libusb_claim_interface(lynsynHandle, 0x1);       // NEW
  if (ret != LIBUSB_SUCCESS) {
    printf("Re-claim interface failed: %s\n", libusb_error_name(ret)); // NEW
    libusb_close(lynsynHandle); lynsynHandle = NULL;     // NEW
    return ret;                                          // NEW
  }

  // Re-detect endpoints (optional but safe)             // NEW
  struct libusb_config_descriptor *config = NULL;        // NEW
  libusb_get_active_config_descriptor(libusb_get_device(lynsynHandle), &config); // NEW
  if (config) {
    struct libusb_interface_descriptor interface = config->interface[1].altsetting[0]; // NEW
    for (int ep = 0; ep < interface.bNumEndpoints; ++ep) {
      if (interface.endpoint[ep].bEndpointAddress & 0x80) inEndpoint = interface.endpoint[ep].bEndpointAddress;
      else outEndpoint = interface.endpoint[ep].bEndpointAddress;
    }
    libusb_free_config_descriptor(config);
  }

  return LIBUSB_SUCCESS;                                  // NEW
}



// === lynsyn_preinit (HARDEN) ===
// // OLD: manual list walk + manual detach + claim
...
//   libusb_set_auto_detach_kernel_driver(lynsynHandle, 1);  // After libusb_open()
...
// NEW: open directly + auto-detach + claim
bool lynsyn_preinit(unsigned maxTries) {
  int r = libusb_init(&usbContext);
  if (r < 0) { printf("libusb_init failed: %s\n", libusb_error_name(r)); return false; }

  unsigned tries = 0;
  while (tries++ < maxTries) {
    lynsynHandle = libusb_open_device_with_vid_pid(usbContext, 0x10c4, 0x8c1e); // NEW
    if (lynsynHandle) break;
    printf("Waiting for Lynsyn device\n"); fflush(stdout); sleep(1);
  }
  if (!lynsynHandle) return false;

  libusb_set_auto_detach_kernel_driver(lynsynHandle, 1);                        // NEW
  int err = libusb_claim_interface(lynsynHandle, 0x1);                          // NEW
  if (err < 0) {
    printf("Could not claim interface 0x1, error %s\n", libusb_error_name(err)); // NEW
    libusb_close(lynsynHandle); lynsynHandle = NULL;                              // NEW
    return false;                                                                 // NEW
  }

  // NEW: fetch endpoints from the active config using our handle
  struct libusb_config_descriptor *config = NULL;                                 // NEW
  libusb_get_active_config_descriptor(libusb_get_device(lynsynHandle), &config);  // NEW
  if (!config) { printf("Could not retrieve active configuration\n"); return false; } // NEW
  struct libusb_interface_descriptor interface = config->interface[1].altsetting[0];  // NEW
  for (int ep = 0; ep < interface.bNumEndpoints; ++ep) {                          // NEW
    if (interface.endpoint[ep].bEndpointAddress & 0x80) inEndpoint = interface.endpoint[ep].bEndpointAddress;
    else outEndpoint = interface.endpoint[ep].bEndpointAddress;
  }
  libusb_free_config_descriptor(config);                                          // NEW
...
void lynsyn_prerelease(void) {
  if (lynsynHandle) {
    libusb_release_interface(lynsynHandle, 0x1);                 // OK if already released
    // Dont forcibly reattach  auto_detach will put it back when handle closes  // NEW
    libusb_close(lynsynHandle);
    lynsynHandle = NULL;                                          // NEW
  }
  if (devs) {                                                     // NEW: only if you kept a list
    libusb_free_device_list(devs, 1);
    devs = NULL;
  }
  if (usbContext) {
...
void sendBytes(uint8_t *bytes, int numBytes) {
  int remaining = numBytes;                              // NEW
  while (remaining > 0) {
    int transferred = 0;                                 // NEW
    int ret = libusb_bulk_transfer(
        lynsynHandle,
        outEndpoint,
        bytes,
        remaining,                                       // NEW: send only remaining
        &transferred,
        USB_IO_TIMEOUT_MS);                              // NEW: no infinite block
    if (ret != 0) {                                      // NEW
      printf("USB OUT transfer error: %s\n", libusb_error_name(ret));
      break;
    }
    if (transferred <= 0) {                              // NEW
      printf("USB OUT transferred 0 bytes\n");
      break;
    }
    remaining -= transferred;                            // NEW
    bytes += transferred;                                // NEW
  }
}
...
bool getBytes(uint8_t *bytes, int numBytes, uint32_t timeout) {
  int transferred = 0;
  if (timeout == 0) timeout = USB_IO_TIMEOUT_MS;           // NEW: avoid infinite block
  int ret = libusb_bulk_transfer(lynsynHandle, inEndpoint, bytes, numBytes, &transferred, timeout);
  if (ret != 0) {
    // optional: clear halt on endpoint
    if (ret == LIBUSB_ERROR_PIPE) {
      libusb_clear_halt(lynsynHandle, inEndpoint);         // NEW
    }
    return false;
  }
  return (transferred == numBytes);
}
// === getArray (ROBUST) ===
// // OLD:
// bool getArray(uint8_t *bytes, int maxNum, int numBytes, unsigned *elementsReceived, uint32_t timeout) {
...
// NEW:
bool getArray(uint8_t *bytes, int maxNum, int numBytes, unsigned *elementsReceived, uint32_t timeout) {
  if (timeout == 0) timeout = USB_IO_TIMEOUT_MS;             // NEW
  int transferred = 0;
  int ret = libusb_bulk_transfer(lynsynHandle, inEndpoint, bytes, maxNum * numBytes, &transferred, timeout);
  if (ret != 0) {
    if (ret == LIBUSB_ERROR_PIPE) libusb_clear_halt(lynsynHandle, inEndpoint); // NEW
    *elementsReceived = 0;                                                     // NEW
    return false;
  }
  if (transferred % numBytes) {                                                // NEW
    *elementsReceived = 0;
    return false;
  }
  *elementsReceived = (unsigned)(transferred / numBytes);                      // NEW
  return true;
}
